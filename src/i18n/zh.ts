export const zh = {
  ...
  demo_summary: '基于**沃伦·巴菲特的价值投资**理念，您的持仓包含优质企业，但科技板块集中度过高（60%）。苹果当前交易价格约 $198.36（P/E 32.1x），微软约 $441.20（P/E 36.8x），NVIDIA 约 $124.58（P/E 64.2x）—— 这些估值均高于历史均值。尽管它们具有持久的竞争护城河（品牌、生态系统、AI 基础设施垄断），但在当前估值水平下，60% 的科技配置已违反安全边际原则。同时，您的固定收益仅占 5%（BND 约 $72.15），远低于稳健配置所需的 15-20%。建议逐步再平衡，纳入低估值板块和保护性固收仓位，以在下一轮市场调整中保持韧性。',
